...
- `POST /chat/api/` — Stream chat response (SSE)
- `GET /chat/history/` — Fetch conversation history
- `GET /chat/debug/` — Chat debug dashboard

### File & Knowledge Management
- `GET /api/files/` — Browse knowledge files
- `GET /api/files/read/<path>/` — Read a knowledge file
- `POST /api/files/write/<path>/` — Write/update a knowledge file

### Project Documentation
- `GET /api/projects/` — List all project documents
- `GET /api/projects/<project_name>/` — Project detail summary
- `GET /api/projects/<project_name>/<doc_type>/` — Retrieve specific project doc
- `POST /api/projects/search/` — Vector search project documentation

### Memory & Lifecycle
- `GET /api/core-memory/` — Core memory view
- `GET /api/core-memory/list/` — List core memory chunks
- `POST /api/memory/<id>/promote/` — Promote chunk to core memory
- `POST /api/memory/<id>/demote/` — Demote chunk from core memory
- `POST /api/memory/<id>/stage/` — Set chunk lifecycle stage
- `GET /api/memory/stage/<stage>/` — Filter chunks by lifecycle stage

### Concept Links & Decisions
- `POST /api/links/create/` — Create concept link
- `GET /api/links/<id>/` — Get links for a chunk
- `GET /api/links/<id>/related/` — Traverse related chunks
- `GET /api/decisions/` — List decision records
- `POST /api/decisions/create/` — Record new decision
- `POST /api/decisions/<id>/status/` — Update decision status

### System & Maintenance
- `GET /system/status/` — System health and statistics
- `POST /system/maintenance/` — Manually trigger memory maintenance cycle
- `POST /system/trigger-scribe/` — Manually trigger Scribe consolidation
- `POST /system/cleanup-files/` — Trigger empty/orphaned file cleanup
- `POST /system/knowledge-maintenance/` — Trigger knowledge gap & contradiction analysis
- `POST /system/purge/` — Factory reset (⚠️ destructive)